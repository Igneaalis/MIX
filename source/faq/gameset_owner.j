        ...
        if (GetTimeInSeconds() < R2I(settingsTimerTime)) then // Shows commands and settings only at game start
            static if not DEBUG_MODE then
                if (pdb[GameOwner].info == true) then // Checks Info flag of game owner
                    ...
                    call DisplayTimedTextToPlayer(GameOwner, 0, 0, settingsTimerTime,  GOLD + "-time ##|r, где ## - время перед началом арены от 20 до 60 сек. " + "Текущий показатель: " + GREEN + I2S(R2I(relaxArenaWaveTime)) + "|r сек.")
                    call DisplayTimedTextToPlayer(GameOwner, 0, 0, settingsTimerTime,  GOLD + "-arena ###|r, где ### - длительность арены от 60 сек. до 300 сек. " + "Текущий показатель: " + GREEN + I2S(R2I(arenaTimerTime)) + "|r сек.")
                    call BuildCommand(GameOwner, "-build")
